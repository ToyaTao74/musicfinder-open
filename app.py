...
APP_VERSION_NAME   = 'v4.30.26 云端抖音取证落地：Xvfb 虚拟显示 + patchright 反爬浏览器 + Mac 登录态随快照分发（headful 反滑块）'
...
def ensure_playwright_chromium(timeout=120, on_status=None):
    ...
    def _have_cli():
        ...
        try:
            v = compute_driver_executable()
            # v4.30.5：新版 playwright 返回 (path, ...) 元组，旧版返回 str——
            # 元组直接传给 subprocess 会炸出 "expected str, bytes or os.PathLike
            # object, not tuple"（Windows 用户实测命中）
            if isinstance(v, tuple):
                v = v[0]
            return v
        except Exception:
            return None
    # v4.30.5：Windows 上不再下载任何浏览器——用系统 Edge（上面已尝试）或手动登录
    if sys.platform.startswith('win'):
        return False, '本机未检测到可用的系统浏览器（Edge/Chrome）。请确认 Edge 可正常打开网页，或改用「打开登录页」手动复制 Cookie'
    cli = _have_cli()
    if not cli:
...
def run_browser_login(platform, timeout=360, target='cookies'):
    ...
    try:
        ...
        launch_errs = []
        browser = None
        # v4.30.5：Windows 上系统 Edge 最优先
        _ch_order = ('msedge', 'chrome', None) if sys.platform.startswith('win') else ('chrome', 'msedge', None)
    ...
    try:
        ...
        for it in tracks:
            ...
            results.append({
                ...
                # v4.29.1：发布时间。汽水 track.album.release_date（Unix 秒）= 专辑发布日，
                # 证据监测页拿它做「视频/作品发布时间」展示与筛选（此前该字段恒为空）
                'release_date': _ts_to_date(album_obj.get('release_date')) or '',
            })
            if len(results) >= max_results:
                break
        return results
...
def _performer_is_alias(perf, res_perf):
    ...
    内部惰性加载白名单，任何调用路径（Web 服务、批量脚本、独立 CLI）都可直接使用
    ——v4.29.4 修复：此前依赖外部先调 _load_performer_aliases()，独立脚本直接
    调用 _pick_best_for_batch 会抛 NameError: _PERFORMER_ALIASES。
    """
...
def _canonical_performer(name):
    ...
    v4.30.7：查询前剥掉尾部标点——平台写法常带尾点/尾空格（实测「Double Face.」
    vs 白名单「Double Face」一字之差归一失败，同一首歌被拆成两行）。
    """
    if not name:
        return name
    n = name.strip().rstrip('.,·!！?？~；;、 ')
...
def _save_marks_cloudbase(marks):
    ...
    try:
        ...
        payloads = []
        for k, v in marks.items():
            # v4.30.22：归属优先取标记自身携带的 username——实测事故：推送统一
            # 盖成当前登录账号，一次同步就把全部标记归属改成当前用户。
            # 当前用户仅作为新标记（无归属信息）的兜底。
            payloads.append({
                'mark_key': k,
                'username': (v.get('username') or '').strip() or u,
                'song_name': v.get('song_name', ''),
                'performer': v.get('performer', ''),
                'album': v.get('album', ''),
                'mark_type': v.get('mark_type', ''),
        ...
        # 3. 只删「当前用户」云端多出的（其他设备已取消的）。
        #    v4.30.21 两道安全护栏（实测事故：云端容器本地标记为空 → cloud-local
        #    = 全部标记被当"多出"删除，135 条标记被清空）：
        #    a) 本地无任何标记时绝不执行 diff 删除（新设备/空环境保护）
        ...
        if not local_keys:
            ...
            extras = list(cloud_keys - local_keys)
            if len(extras) > 20:
                logger.error('[marks] 云端 diff 待删 %d 条超过护栏(20)，拒绝执行！'
                             'local=%d cloud=%d', len(extras), len(local_keys), len(cloud_keys))
            else:
                for extra in extras:
                    try:
...
def api_diag_auth():
    """登录排障诊断（v4.30.15）：只暴露计数与布尔值，不含任何密码数据。"""
    out = {}
    try:
        out['cloud_cfg'] = bool(_cloud_auth_cfg())
    except Exception as e:
        out['cloud_cfg'] = f'ERR:{str(e)[:80]}'
    try:
        c = _cloud_load_users()
        out['cloud_users'] = len(c) if isinstance(c, dict) else repr(c)[:40]
        if isinstance(c, dict) and '杨颖大王' in c:
            out['cloud_yy_pw_len'] = len((c['杨颖大王'] or {}).get('pw') or '')
    except Exception as e:
        out['cloud_users'] = f'ERR:{str(e)[:80]}'
    try:
        local = _load_users_auth()
        lu = local.get('users') if isinstance(local.get('users'), dict) else local
        out['local_users'] = len(lu or {})
        if '杨颖大王' in (lu or {}):
            out['local_yy_pw_len'] = len((lu['杨颖大王'] or {}).get('pw') or '')
            out['local_yy_disabled'] = bool((lu['杨颖大王'] or {}).get('disabled'))
    except Exception as e:
        out['local_users'] = f'ERR:{str(e)[:80]}'
    return jsonify(out)
